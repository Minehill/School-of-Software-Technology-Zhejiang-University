...
class MultiSpike(nn.Module):
    def __init__(
        ...
        if Norm is None:
            initial_norm_value = float(self.max_value)
        else:
            initial_norm_value = float(Norm)
            
        # 直接定义为一个可学习的nn.Parameter
        self.learnable_norm = nn.Parameter(torch.tensor([initial_norm_value]))
    ...
    def __repr__(self):
        # 更新 repr 以显示这是一个可学习的Norm
        return f"MultiSpike(LearnableNorm={self.learnable_norm.item():.4f}, Max_Val={self.max_value})"

    def forward(self, x):
        # 使用可学习的Norm进行归一化
        norm_val = torch.clamp(self.learnable_norm, min=1e-8)
        return self.spike_function(x, min_value=self.min_value, max_value=self.max_value) / norm_val
