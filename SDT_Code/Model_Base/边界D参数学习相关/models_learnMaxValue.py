...
class MultiSpike(nn.Module):
    def __init__(
        ...
        max_value=4.0,
        Norm=None,
        learn_max_value=True,  # 默认开启学习
        ):
        super().__init__()
        self.min_value = min_value
        self.learn_max_value = learn_max_value

        if self.learn_max_value:
            # 将max_value定义为可学习参数
            self.max_value = nn.Parameter(torch.tensor(float(max_value)))
        else:
            self.max_value = max_value
        
        if Norm is None:
            self.Norm = float(max_value)
        else:
            self.Norm = float(Norm)
    
    def forward(self, x):
        # 获取当前的最大值
        current_max_value = self.max_value

        # 如果max_value是可学习的Tensor，则min_value也需要是Tensor
        # 这是为了解决 `clamp(Tensor, min=float, max=Tensor)` 的TypeError
        if isinstance(current_max_value, torch.Tensor):
            # 将min_value转换为与max_value相同设备和类型的Tensor
            current_min_value = torch.tensor(self.min_value, device=current_max_value.device, dtype=current_max_value.dtype)
            # 确保max_value不会小于min_value
            current_max_value = torch.max(current_min_value, current_max_value)
            
            quantized_x = Quant.apply(x, current_min_value, current_max_value)
        else:
            # 如果max_value不是Tensor，则一切照旧
            quantized_x = Quant.apply(x, self.min_value, self.max_value)
        
        return quantized_x / self.Norm
        
    def __repr__(self):
        if self.learn_max_value:
            # 使用 .data 访问Parameter的值
            return f"MultiSpike(LearnableMaxVal={self.max_value.data.item():.4f}, Norm={self.Norm})"
        else:
            return f"MultiSpike(Max_Val={self.max_value}, Norm={self.Norm})"
