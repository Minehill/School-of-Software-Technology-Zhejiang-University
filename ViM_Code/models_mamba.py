...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        # batch_size, patchs数量, token长度
        B, M, _ = x.shape

        if self.if_cls_token:
            if self.use_double_cls_token:
                # cls_token_head/tail 在 init中初始化为[1, 1, token长度]
                # 经过expand，它被复制了B次,因此现在的维度为(batch_size, 1, token长度)
                cls_token_head = self.cls_token_head.expand(B, -1, -1)
                cls_token_tail = self.cls_token_tail.expand(B, -1, -1)
                # 计算pathch序列中间位置
                token_position = [0, M + 1]
                # 在token序列的头尾都拼接上,维度为[batch_size, 1+patch数量+1，token长度]
