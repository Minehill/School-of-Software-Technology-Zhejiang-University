...
# 多尺度mamba
import models_hierarchical_mamba
...
def param_grad_hook_to_file(grad, name):
    # 从全局字典中获取当前状态
    log_path = grad_log_info["log_path"]
    epoch = grad_log_info["epoch"]
    step = grad_log_info["step"]

    if grad is not None and log_path is not None:
        if torch.isnan(grad).any() or torch.isinf(grad).any():
            norm_str = "NaN_or_Inf"
        else:
            norm_str = f"{grad.norm().item():.6f}"

        log_message = f"E:{epoch}, S:{step}, P:{name}, GradNorm:{norm_str}\n"
        try:
...
def main(args):
    ...
    if TRACK_GRADIENTS:
        ...
        # 将日志路径存入全局字典
        grad_log_info["log_path"] = output_dir / f"grad_log_rank{utils.get_rank()}.txt"
        
        if args.start_epoch == 0 and utils.get_rank() == 0:
            if grad_log_info["log_path"].exists():
                open(grad_log_info["log_path"], 'w').close()
        
        print(f"Rank {utils.get_rank()}: Tracking gradients to {grad_log_info['log_path']}")

        # 注册 hook
        for name, p in model.named_parameters():
            if p.requires_grad:
                p.register_hook(lambda grad, name=name: param_grad_hook_to_file(grad, name))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args=args,
            grad_log_info=grad_log_info if TRACK_GRADIENTS else None,
        )
